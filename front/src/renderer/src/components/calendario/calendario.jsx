	...
	switch (escala) {
		...
		case "12 x 36":
			...
			for (let i = 0; i < 365; i += 2) {
				// Incrementa de 2 em 2 dias para cada ciclo de trabalho+folga
		...
		case "24 x 72":
			...
			for (let i = 0; i < 365; i += 4) {
				// Incrementa de 4 em 4 dias para cada ciclo de trabalho+folga
				const currentDate = startOfDay.add(i, "day"); // Calcula o dia atual de trabalho
				workDays.add(currentDate.format("YYYY-MM-DD")); // Adiciona o dia de trabalho
			}
			break;
		case "8 x 40":
			...
			for (let i = 0; i < 365; i += 7) {
				...
				if (Math.floor(i / 7) % 2 === 0) {
					// Semana par: segunda, quarta, sexta
					...
					// Semana ímpar: terça, quinta, sábado
		...
		case "12 x 60":
			...
			for (let i = 0; i < 365; i += 3) {
				// Incrementa i por 3 para cada ciclo de trabalho+folga
				const currentDate = startOfDay.add(i, "day");
...
export function DateCalendarServerRequest({ user }) {
	...
	const markedDays = React.useMemo(() => {
		...
		if (user.dataInicio && user.dataTermino) {
			...
			allHighlightedDays = [...allHighlightedDays, ...combinedHighlightedDays];
		}
